import vidstackBaseStyles from '@vidstack/react/player/styles/base.css?inline';
...
import { VideoLayout } from './components/layouta/VideoLayout1'; //cái này có dùng tailwind có lỗi 
import { textTracks } from './Tracks';
import { RotateIcon } from '@vidstack/react/icons';
import root from 'react-shadow';
import example from './components/layouta/example.css?inline'
import sub from './components/layouta/childs/sub.css?inline'
// // 🛠️ SỬA ĐỔI CHÍNH: Nhập trực tiếp mã nguồn CSS gốc của Tailwind v4 từ node_modules dạng ?raw (Không dùng link https)
import tailwindV4Styles from '../tailwind.css?inline';
import watchcss from '../../public/css/watch.css?raw';

export function Player({ src, poster }) {
  // Loại bỏ định nghĩa kiểu dữ liệu TypeScript <MediaPlayerInstance>
  ...
  // Cấu hình engine HLS (Bỏ các khai báo kiểu nativeEvent)
  function onProviderChange(provider) {
    if (isHLSProvider(provider)) {
      // Bạn có thể cấu hình hls.js nâng cao tại đây nếu cần
      provider.config = {
        maxBufferLength: 30,
  ...
  function onCanPlay(detail) {
    // Xử lý logic khi video đã sẵn sàng phát
  }

  // Khai báo link linh hoạt: ưu tiên prop truyền vào, nếu không có sẽ lấy link gốc Sprite Fright
  const videoSrc = src || "https://s4.phim1280.tv/20250317/05z7tNIU/index.m3u8";
  const posterSrc = poster || "/stickers/wangling_poster.webp";

  return (
    <>
      {/* <style>{tailwindV4Styles}</style> */}
      {/* <style>{vidstackBaseStyles}</style> */}
      <style>{example}</style>
      <style>{sub}</style>
      {/* <style>{watchcss}</style> */}
      <MediaPlayer
        part="media-player"
        data-media-player
        ...
        onCanPlay={() => {
          window.dispatchEvent(new CustomEvent('video-loading-state', { detail: { loading: false } }));
        }}
        onWaiting={() => {
          window.dispatchEvent(new CustomEvent('video-loading-state', { detail: { loading: true } }));
        }}
        onPlaying={() => {
          window.dispatchEvent(new CustomEvent('video-loading-state', { detail: { loading: false } }));
        }}
      ...
      >
        ...
        {/* KHỐI LOADING OVERLAY 2 DÀNH CHO FULL SCREEN TRONG SHADOW DOM              */}
    ...
    /* MẸO EXPERT: Khi MediaPlayer kích hoạt trạng thái buffering, lập tức HIỆN overlay này lên, body:has(media-player[data-buffering]) .loading-overlay */
