...
const EPISODES_MOCK = Array.from({ length: 15 }, (_, i) => ({
  id: 149 - i,
  title: `Tập ${149 - i}`,
  thumb: 'https://unsplash.com'
}));

export default function EpisodeSidebar({ titleMovie = "Tiên Nghịch" }) {
  return (
    <div className="relative w-full h-full bg-black overflow-hidden font-sans text-white select-none">
      ...
      <div className="absolute inset-0 bg-zinc-900 flex items-center justify-center">
        <span className="text-zinc-500 text-sm">Trình phát video chính (Video Container)</span>
      </div>
      ...
      {/* 2. LỚP OVERLAY MỜ NỀN KHI MỞ SIDEBAR (ĐÚNG NHƯ ẢNH MẪU) */}
      {/* Mặc định ẩn (opacity-0 pointer-events-none), khi input checked sẽ hiện và làm mờ */}
      <label 
        htmlFor="sidebar-toggle-native"
      ...
      {/* h-full giúp sidebar luôn cao bằng 100% khung container bao quanh nó */}
      <div className="absolute top-0 right-0 h-full w-[280px] sm:w-[320px] bg-[#0c0c0e]/95 border-l border-white/5 z-50 flex flex-col translate-x-full peer-checked/sidebar:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl">
        ...
        <div className="flex gap-2 px-4 pb-4 border-b border-white/5">
          ...
          <div className="relative flex-1 group/dropdown" tabIndex="0">
            <button className="w-full flex items-center justify-between bg-[#1c1c22] hover:bg-[#25252d] px-2.5 py-1.5 rounded text-xs font-medium border border-white/5 text-[#d1d5db] cursor-pointer">
              <span>☰ Phần Chính</span>
              <span className="text-[9px] opacity-50 transition-transform duration-200 group-focus-within/dropdown:rotate-180">▼</span>
            </button>
            <div className="absolute top-[calc(100%+4px)] left-0 flex flex-col bg-[#141419] border border-white/10 rounded p-1 min-w-[130px] shadow-2xl z-50 invisible opacity-0 scale-95 origin-top transition-all duration-150 group-focus-within/dropdown:visible group-focus-within/dropdown:opacity-100 group-focus-within/dropdown:scale-100">
              {SEASONS_MOCK.map((item) => (
                <button key={item.value} className={`w-full text-left text-xs px-3 py-2 rounded-sm cursor-pointer hover:bg-white/10 outline-none ${item.value === 'main' ? 'text-orange-500 font-semibold' : 'text-white/80'}`}>
                  {item.label}
                </button>
              ))}
            </div>
          ...
          <div className="relative flex-1 group/dropdown" tabIndex="0">
            ...
            <div className="absolute top-[calc(100%+4px)] right-0 flex flex-col bg-[#141419] border border-white/10 rounded p-1 min-w-[110px] shadow-2xl z-50 invisible opacity-0 scale-95 origin-top transition-all duration-150 group-focus-within/dropdown:visible group-focus-within/dropdown:opacity-100 group-focus-within/dropdown:scale-100">
              {SUB_MOCK.map((item) => (
                <button key={item.value} className={`w-full text-left text-xs px-3 py-2 rounded-sm cursor-pointer hover:bg-white/10 outline-none ${item.value === 'vi' ? 'text-orange-500 font-semibold' : 'text-white/80'}`}>
                  {item.label}
                </button>
              ))}
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="relative w-20 h-11 bg-zinc-800 rounded overflow-hidden flex-shrink-0 border border-white/5">
                  <img src={ep.thumb} alt={ep.title} className="w-full h-full object-cover opacity-60" />
                ...
                {/* Tiêu đề tập phim */}
                <span className="text-xs md:text-sm tracking-wide">{ep.title}</span>
              </div>
            );
          })}
        </div>
