        ...
        <>
            ...
            >
                <div className={`wpd-comment-wrap wpd-blog-user ${blogStyleClass}`}>
                    ...
                    <div className="wpd-comment-left ">
                        ...
                        {cmt.ring && (
                            <div
                                ...
                                style={{ cursor: 'pointer', '--ring-cmt-w': cmt.ring.ringW || '50px' }}
                        ...
                        {cmt.hongNhan && (
                            <div
                                ...
                                style={{ cursor: 'pointer', '--ring-cmt-w': cmt.hongNhan.ringW || '50px' }}
                                onClick={e => { const m = document.querySelector('.hn-modal'); m && (m.style.display = 'flex') }}
                            >
                                <img
                    ...
                    {/* ==================== CỘT BÊN PHẢI ==================== */}
                    <div id={`comment-${cmt.id}`} className="wpd-comment-right" style={{ '--vip-bg-url': cmt.vipBgUrl }}>
                ...
                {cmt.replies && cmt.replies.length > 0 && ReplyItem && (
                    <div className="wpd-reply-responses">
                        {cmt.replies.map((reply) => (
                            <ReplyItem
                                ...
                                reply={reply}
                                parentId={cmt.id}
                                onMouseEnter={(e, id) => handleMouseEnter && handleMouseEnter(e, id)}
                                onMouseLeave={handleMouseLeave}
                                onVote={(type, id) =>
                                    handleAddVotes1 && handleAddVotes1(type, id, setComments)
                                }
                                onToggleReply={(id) => handleToggleReply && handleToggleReply(id)}
                                nextReply={{
                                    setIsOpen: setIsOpen,
                                    setOnSelectStickerCallback: setOnSelectStickerCallback,
                                    onAddComment: handleAddComment,
