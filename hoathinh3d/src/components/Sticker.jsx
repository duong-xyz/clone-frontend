...
// Dữ liệu mẫu (Thay thế bằng gọi API thật nếu bạn có backend)
...
const Sticker = ({ isOpen, setIsOpen, onSelectSticker }) => {
  ...
  // Khi bấm chuyển Tab
  const handleTabChange = (packId) => {
    if (packId === activeTab) return;
    setIsLoading(true);
    setActiveTab(packId);

    // Giả lập delay mạng nhẹ khi tải sticker (nếu cần)
    setTimeout(() => {
      setIsLoading(false);
    }, 200);
  };
  ...
  const handleSelectSticker = (sticker) => {
    ...
    const filteredRecent = recentStickers.filter((item) => item.id !== sticker.id);
    const updatedRecent = [sticker, ...filteredRecent].slice(0, 15);
    
    setRecentStickers(updatedRecent);
    localStorage.setItem("wpd_recent_stickers", JSON.stringify(updatedRecent));

    // B. Trả dữ liệu ra ngoài cho CommentForm xử lý (chèn hình vào ô nhập hoặc gửi trực tiếp)
    if (onSelectSticker) {
